            ...
            if (revents & posix.POLL.IN != 0) {
                if (clients.getPtr(polled.fd)) |client| {
                    ...
                    if (client.buffer.len > 4 and client.buffer.multi_size > 0 and client.buffer.data[multi_size_str_len..client.buffer.len].len == client.buffer.multi_size + 2) {
                        result = handleConnection(client, store, client.buffer.data[multi_size_str_len..client.buffer.len], app_config);
                        client.buffer.reset();
                    }
                    if (client.buffer.multi_size == 0) {
                        result = handleConnection(client, store, client.buffer.items(), app_config);
                        client.buffer.reset();
                    }
